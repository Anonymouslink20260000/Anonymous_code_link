...
Optimizes prompts using multiple LLM models (Qwen API, OpenAI GPT, Anthropic Claude, and Ollama local models)
...
# Try to import Anthropic Claude support
try:
    from langchain_anthropic import ChatAnthropic
    ANTHROPIC_AVAILABLE = True
except ImportError:
    ANTHROPIC_AVAILABLE = False
    print("Warning: langchain-anthropic not installed. Claude models will not be available.")
...
# model_type: "qwen" for Qwen API, "ollama" for Ollama local models, "gpt" for OpenAI GPT, "claude" for Anthropic Claude
DEFAULT_MODELS = [
    ("llama3:8b", "ollama"),
    ("qwen-max", "qwen"),
    ("qwen3:4b", "ollama"),
...
OPENAI_API_BASE = os.getenv("OPENAI_API_BASE", "https://api.openai.com/v1")  # Optional: for custom endpoints
ANTHROPIC_API_KEY = os.getenv("ANTHROPIC_API_KEY", "")  # Set via environment variable
...
def create_llm(model_name: str, model_type: str = "qwen") -> Union[BaseChatModel, ChatOpenAI]:
    """
    Create LLM instance based on model type.
    
    Args:
        model_name: Name of the model
        model_type: Type of model ("qwen", "ollama", "gpt", or "claude")
        
    Returns:
        LLM instance (ChatOpenAI or ChatAnthropic)
        
    Raises:
        ValueError: If model_type is unknown or required API key is not set
    """
    if model_type == "qwen":
        if not QWEN_API_KEY:
        ...
        # Ollama local model configuration (supports DeepSeek and other Ollama models)
        return ChatOpenAI(
        ...
        if not ANTHROPIC_AVAILABLE:
            raise ValueError("langchain-anthropic package is not installed. Install with: pip install langchain-anthropic")
        ...
        raise ValueError(f"Unknown model type: {model_type}. Supported types: qwen, ollama, gpt, claude")
...
def optimize_prompt_with_model(
    prompt: str,
    llm: Union[BaseChatModel, ChatOpenAI],
    model_name: str,
